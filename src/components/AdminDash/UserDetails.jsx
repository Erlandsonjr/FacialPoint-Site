...
import { FaArrowLeft, FaUserCircle, FaCalendarCheck, FaCalendarTimes, FaClock, FaExclamationCircle } from 'react-icons/fa';
...
function UserDetails() {
    ...
    const fetchUserDetails = async () => {
        try {
            ...
            const userResponse = await fetch(`https://faceponto-banco-dados-production.up.railway.app/usuarios/${id}`, {
                headers: {
            ...
            // Buscar frequências do usuário
            const frequenciasResponse = await fetch(`https://faceponto-banco-dados-production.up.railway.app/frequencias/usuario/${id}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
                    ...
                    <div className="profile-header">
                        ...
                        <div className="profile-info">
                            ...
                            <p className="profile-status">
                                Status: <span className={user?.ativo ? "status-active" : "status-inactive"}>
                        ...
                        <div className="details-section">
                            ...
                            <div className="details-grid">
                                ...
                                <div className="detail-item">
                                    ...
                                    <span className="detail-value">{user?.role || "Funcionário"}</span>
                                </div>
                                <div className="detail-item">
                                    <span className="detail-label">Horário de Entrada:</span>
                                    <span className="detail-value">{user?.horario_entrada || "08:00"}</span>
                                </div>
                                <div className="detail-item">
                                    <span className="detail-label">Horário de Saída:</span>
                                    <span className="detail-value">{user?.horario_saida || "17:00"}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    {frequencias.map(freq => (
                                        <tr key={freq._id}>
                                            ...
                                            <td>
                                                <span className={`badge ${freq.tipo_registro === 'entrada' ? 'badge-entry' : 'badge-exit'}`}>
                                                    {freq.tipo_registro === 'entrada' ? 'Entrada' : 'Saída'}
                                                </span>
                                            </td>
                                        </tr>
                                    ))}
